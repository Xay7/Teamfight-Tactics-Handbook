  ...
  return (
    ...
    >
      ...
      <View style={styles.championsContainer}>
        {championList.map(el => {
          if (
            props.name === champions[el].origin ||
            props.name === champions[el].originSecond
          ) {
            return (
              <TouchableOpacity
                onPress={() =>
                  props.navigation.navigate("ChampionInfo", {
                    name: el
                  })
                }
                key={el}
